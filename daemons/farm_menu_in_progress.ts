	...
	setVar $BOT~help[15] $BOT~tab&"        [port] - will create and upgrade sbb on ports"
	setVar $BOT~help[16] $BOT~tab&"    [skipfuel] - won't upgrade fuel       - for port option"
	setVar $BOT~help[17] $BOT~tab&"     [skiporg] - won't upgrade organics   - for port option "
	setVar $BOT~help[18] $BOT~tab&"   [skipequip] - won't upgrade equipment  - for port option"
	setVar $BOT~help[19] $BOT~tab&"       [build] - will create and upgrade planets and port"
	setVar $BOT~help[20] $BOT~tab&"[destroyports] - will destroy all non-class 3 ports during build"
	setVar $BOT~help[21] $BOT~tab&"         [one] - only pops 1 planet when none exist during build"
	setVar $BOT~help[22] $BOT~tab&"     [nostrip] - won't attempt to strip during build"
	setVar $BOT~help[23] $BOT~tab&"   [noupgrade] - won't attempt to upgrade during build"
	setVar $BOT~help[24] $BOT~tab&"     [defense] - set cannons"
	setVar $BOT~help[25] $BOT~tab&"      [amtrak] - uses amtrak sectors as farm sectors"
	setVar $BOT~help[26] $BOT~tab&"  [allplanets] - uses tl sectors as farm sectors"
	setVar $BOT~help[27] $BOT~tab&"     [planets] - only visits sectors with planets"
	setVar $BOT~help[28] $BOT~tab&"      [bubble] - uses BUBBLE sectors as farm sectors"
	setVar $BOT~help[29] $BOT~tab&"     [balance] - balance planets throughout bubble or farm"
	setVar $BOT~help[30] $BOT~tab&"     [movefig] - moves all fighters from planet to sector"
	setVar $BOT~help[31] $BOT~tab&"   [barricade] - moves all fighters from planets to home sector"
	setVar $BOT~help[32] $BOT~tab&"  [armageddon] - destroys all planets and ports (kills colos)"
	setVar $BOT~help[33] $BOT~tab&"         [off] - turns off farming script"
	setVar $BOT~help[34] $BOT~tab&"       "
	setVar $BOT~help[35] $BOT~tab&"Data Options:"
	setVar $BOT~help[36] $BOT~tab&"         {set} plus [sector number]"
	setVar $BOT~help[37] $BOT~tab&"               - Marks sector as a FARM Sector"
	setVar $BOT~help[38] $BOT~tab&"                 "
	setVar $BOT~help[39] $BOT~tab&"       {clear} - Clears all FARM and BUBBLE sectors"
	setVar $BOT~help[40] $BOT~tab&"              "
	setVar $BOT~help[41] $BOT~tab&"   {setbubble} plus [bubble door]  "
	setVar $BOT~help[42] $BOT~tab&"               - sets all BUBBLE sectors"
	setVar $BOT~help[43] $BOT~tab&"                         "
	setVar $BOT~help[44] $BOT~tab&"        {list} - Lists all FARM sectors"
	setVar $BOT~help[45] $BOT~tab&" {list bubble} - Lists all BUBBLE sectors"
	setVar $BOT~help[46] $BOT~tab&"        "
	setVar $BOT~help[47] $BOT~tab&"       {remove} plus [sector number]"
	setVar $BOT~help[48] $BOT~tab&"               - Removes FARM marked sector"
	...
	if ($bot~parm1 = "menu")
		setVar $SWITCHBOARD~message "Opening up Farm Menu. . .*"
		gosub :SWITCHBOARD~switchboard
		setarray $bot~menu 30 1
		setvar $bot~menu[1]  "Balance"
		setvar $bot~menu[1][1] "This is to balance all planets in targeted sectors"
		setvar $bot~menu 1
		gosub :bot~menu
	end
	:start_farm
...
:tryAgain
	...
	while (($farmSector <= SECTORS) and ($farmSector > 0))
		...
		if ($where_planets = true)
			if ($planet~CITADELs[$farmsector] <= 0)
...
:stripallplanets
	...
	while ($j <= $planet~planetCount)
		if ($planet~planetToFill <> $planet~planets[$j])
		        		...
		        		gosub :setWindow
                        if (($planet~planet_CITADEL_CREDITS > 0) and ($cash))
							if ($planet~planet_CITADEL_CREDITS > 999999999) or (($planet~planet_CITADEL_CREDITS +  $PLAYER~CREDITS) > 999999999)
							    setVar $planet~planet_CITADEL_CREDITS (999999999 - $PLAYER~CREDITS)
							else
						        setVar $planet~planet_CITADEL_CREDITS ($planet~planet_CITADEL_CREDITS + $PLAYER~CREDITS)
							end
							send "t f " & $planet~planet_CITADEL_CREDITS & "* qq* l " & #8 & $planet~planetToFill & "* c t t " & $planet~CITADELCash & "* qq* "
                        end
                        if (($shield) and ($planet~planet_CITADEL > 4) and ($planet~planet_SHIELD_POWER < 200))
                            if ($PLAYER~SHIELDS < 2000)
                                   ...
                                   send "l " & #8 & $planet~planets[$j] & "* c gt200*"
                            end
                        end
                        if (($warp = TRUE) and ($planet~planet_CITADEL > 3) and ($planet~planet_FUEL > 5000) and (($planet~planet_ORGANICS > 5000) or ($planet~planet_EQUIPMENT > 5000)))
							send "qq* l " & #8 & $planet~planets[$j] & "* c "
							gosub :merch
							send "d"
...
:menuSpacing
    ...
    while ($qss_count <= $qss_total)
        setVar $spc_count 1
        setVar $checkLength $labels[$qss_count]&""&$values[$qss_count]
        setVar $label_values[$qss_count] ANSI_15&$labels[$qss_count]&" "&ANSI_14&$values[$qss_count]&ANSI_7
        getLength $checkLength $length
        setVar $space 34
        subtract $space $length
        while ($spc_count <= $space)
            mergeText $label_values[$qss_count] $spc $label_values[$qss_count]
            add $spc_count 1
        end
        add $qss_count 1
    end
...
include "source\module_includes\bot\menu\bot"
