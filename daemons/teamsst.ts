...
#	In the HHT we will get to many holds, i.e. 250 hold ship, so we need to get experience, and calc holds and max em out
#		OR we manual run
#		Or we buy the amount of holds we bust with
	...
	setArray $BLUES $MAX_BLUE_BOTS
	 ...
	 setVar $BOT~help[1] $BOT~tab&"Steal, Sell, Transport with multiple bots "
	 setVar $BOT~help[2] $BOT~tab&"	   "
	 setVar $BOT~help[3] $BOT~tab&" Start Evils in their spare ships, different sector."
	 setVar $BOT~help[4] $BOT~tab&"Options: "
	 setVar $BOT~help[5] $BOT~tab&"         [ship1] - ship id of first cashing ship "
	 setVar $BOT~help[6] $BOT~tab&"         [ship2] - ship id of second cashing ship "
	 setVar $BOT~help[7] $BOT~tab&"         [ship3] - ship id of third cashing ship "
	 setVar $BOT~help[8] $BOT~tab&"        {override} - won't stop if blue not fedsafe"
	 setVar $BOT~help[9] $BOT~tab&"        {planetfuel} - will grab fuel first planet when furbing"
	 setVar $BOT~help[10] $BOT~tab&"        {jet} - direct reds to SST and Jet"
	 setVar $BOT~help[11] $BOT~tab&"        {xfurb} - fly cash ship to Class 0 for furb"
	 setVar $BOT~help[12] $BOT~tab&"        {ep} - direct bots to us EP Haggle"
	...
	if (($startingLocation <> "Citadel") AND ($startingLocation <> "Command"))
		setVar $SWITCHBOARD~MESSAGE "Team SST controller must be run from Command or Citadel prompt.*"
		gosub :SWITCHBOARD~SWITCHBOARD
		halt
	end

	setVar $BOT~script_title "Team SST"
	gosub :BOT~banner
	...
	if ($red_count <= 1)
		setVar $SWITCHBOARD~MESSAGE "Found "&$red_count&" red. Not enough reds to run Team SST.  Make sure red bots callin as red1, red2, etc.*"
		gosub :SWITCHBOARD~SWITCHBOARD
		halt
	end
	...
	while (($i <= $MAX_BLUE_BOTS) AND ($roll_call_done = FALSE))
		...
		:foundblue
			...
			if ($blue_sector <> $MAP~STARDOCK)
				if (($blue_sector = 1) and ($xfurb = TRUE))
	...
	if ($blue_count < 1)
		setVar $SWITCHBOARD~MESSAGE "Found "&$blue_count&" blue. Not enough blue to run Team SST.  Make sure blue bots callin as blue1, blue2, etc.*"
		gosub :SWITCHBOARD~SWITCHBOARD
		halt
	end
	...
	send "x"
	:startx
	settextlinetrigger     shipnumber     :xstart "-----------------------------------------------------------------------------"
	settextlinetrigger     doneships      :xfed "nmanned ships in FedSpace will be automatical"
	pause
	:xfed
		killalltriggers
		send "*"
	...
	settextlinetrigger     doneships      :check_bot_status "Command ["
	pause
	...
	:check_bot_status
		...
		while ($keep_going = TRUE)
			...
			while ($i <= $red_count)
				...
				setVar $SWITCHBOARD~MESSAGE $SWITCHBOARD~MESSAGE&" TURNS: "&$RED_TURNS[$i]&"*"
				add $i 1
			end
			gosub :SWITCHBOARD~SWITCHBOARD
			...
			setVar $ordersrepeat 0
			:epfailrecov
			send "'red"&$orders[1]&" SST "&$orders[1][1]&" "&$orders[2][1]&" "&$orders[1][2]&" "&$orders[2][2]&$jet&$ep&"*"
			:repeatorders
			...
			settextlinetrigger noexp :noexp "You need more experience to SST!!!"
			...
			settextlinetrigger epfail :epfail "Ep Haggle timed out"
			pause
			:epfail
				setVar $SWITCHBOARD~MESSAGE "EP Haggle has died on this bot*"
				gosub :SWITCHBOARD~SWITCHBOARD
				setVar $SWITCHBOARD~MESSAGE "Will restart cashers EP Haggle in one moment*"
				gosub :SWITCHBOARD~SWITCHBOARD
				setVar $ordersrepeat 0
				setDelayTrigger failp :failp 2500
				pause
				:failp
				send "'red"&$orders[1]&" epman start*"
				waitfor "EP Haggle started!"
			...
			:lraship
				...
				gosub :SWITCHBOARD~SWITCHBOARD
				send "'red"&$orders[1]&" SST "&$orders[2][1]&" "&$orders[1][1]&" "&$orders[2][2]&" "&$orders[1][2]&$jet&$ep&"*"
			...
			:setupfurber
				...
				if ($xfurb = TRUE)
					:xport
						...
						settexttrigger xportx :donex  "- Xport complete."
						pause
						:donex
							killalltriggers
				
					
					send "'blue1 xfurb "&$bust_ship&" "&$FURB_HOLDS&" "&$FURB_SHIP&"  *"
					settexttrigger xfurb1 :xfurb1 "xfurb complete"
					setDelayTrigger delayx :waitforxfurb 15000
					pause
					:waitforxfurb
						killalltriggers
						setVar $SWITCHBOARD~MESSAGE "XFurb Time Out - send 'GO GO GO' without the spaces to restart after you manually furb.*"
						gosub :SWITCHBOARD~SWITCHBOARD
						setVar $SWITCHBOARD~MESSAGE "Reset the ships, kill xfurb and go!.*"
						gosub :SWITCHBOARD~SWITCHBOARD
						waitfor "GOGOGO"
						goto :xfurbdone
					:xfurb1
					...
					:nofig
						killalltriggers
						send "'no figs? Grimy will be happy!! sort this out and send manu furb command*"
						goto :furbtryagain
					:furb1
						killalltriggers
