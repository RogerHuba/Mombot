...
echo ANSI_11 "*S" ANSI_10 " - Send MCIC data to subspace."
echo ANSI_11 "*C" ANSI_10 " - Change Min MCIC Check."
...
:printMCIC
killalltriggers
echo ANSI_11 "**Displaying all stored MCIC information with MCIC > 59*"
setvar $count 11
while ($count <= SECTORS)
...
:subspaceMCIC
killalltriggers
send "'*Displaying all stored MCIC information with MCIC > 59*"
setvar $count 1
while ($count <= SECTORS)
	# has to search all sectors beacuse you might import ports from corpies
	# that you have not explored yet.  slower, but what are ya gonna do?
...
if ($portmaxinit >= 1300) 
	if ((PORT.BUYORG[CURRENTSECTOR] = 1) OR (PORT.BUYEQUIP[CURRENTSECTOR] = 1))
		setsectorparameter CURRENTSECTOR "GOODPORT" TRUE
	end
end
if ($portmaxinit >= 1393)
	setVar $MCIC "-65"
