	...
	setVar $BOT~help[6]    $BOT~tab&" {holokill} - attempts to holokill after drop (direct or plock)"
	setVar $BOT~help[7]    $BOT~tab&"   {direct} - try to drop directly into the sector"
	setVar $BOT~help[8]    $BOT~tab&"   {return} - after drop, return to starting sector "
	setVar $BOT~help[9]    $BOT~tab&"              and scan again"
	setVar $BOT~help[10]   $BOT~tab&"    {delay} - how many milliseconds to wait before drop or plock"
	setVar $BOT~help[11]   $BOT~tab&"   {figs:n} - drop this many figs to sector on landing"
	setVar $BOT~help[12]   $BOT~tab&"{offensive} - make figs offensive, default defense."
	setVar $BOT~help[13]   $BOT~tab&"              Will ship drop if started in command prompt."
	gosub :bot~helpfile
