	...
	setVar $BOT~help[1]  $BOT~tab&"              <<<< fuel mow ish >>>>"
	setVar $BOT~help[2]  $BOT~tab&" "
	setVar $BOT~help[3]  $BOT~tab&" Moves sector by sector till it has fuel and mows rest."
	setVar $BOT~help[4]  $BOT~tab&" Does not use CIM data, made for day 1 use."
	setVar $BOT~help[5]  $BOT~tab&" "
	setVar $BOT~help[6]  $BOT~tab&" fmow [destination] {figs} {kill} {cap} {saveme} {p} {back}"
	setVar $BOT~help[7]  $BOT~tab&"                   {personal} {backdoor} {i1/i2/i3} "
	setVar $BOT~help[8]  $BOT~tab&"                    "
	setVar $BOT~help[9]  $BOT~tab&" Options:"
	setVar $BOT~help[10]  $BOT~tab&"        {p} - port ship immediately upon arrival."
	setVar $BOT~help[11]  $BOT~tab&"     {kill} - attempt to kill immediately upon arrival."
	setVar $BOT~help[12]  $BOT~tab&"      {cap} - attempt to capture immediately upon arrival."
	setVar $BOT~help[13]  $BOT~tab&"   {saveme} - call saveme to be picked up at destination."
	setVar $BOT~help[14]  $BOT~tab&"     {back} - twarp back to start sector after mow"
	setVar $BOT~help[15]  $BOT~tab&"   {hoover} - attempts to pull fighters from sectors "
	setVar $BOT~help[16]  $BOT~tab&" {personal} - drops personal fighters instead of corp  "
	setVar $BOT~help[17]  $BOT~tab&" {backdoor} - mow to sector via backdoor"
	setVar $BOT~help[18]  $BOT~tab&" {i1/i2/i3} - Indirect mow, void 1-3 sectors"
	...
	if ($mow_SHIP_MAX_ATTACK < 99) or ($PLAYER~FIGHTERS < 99)
		...
		while ($j <= $PLAYER~courseLength)
			if ($PLAYER~mowCourse[$j] <> $PLAYER~CURRENT_SECTOR)
				...
				setVar $waitPort 0
				if (($PLAYER~mowCourse[$j] > 1) AND ($PLAYER~mowCourse[$j] <> $MAP~stardock) AND ($j < $PLAYER~courseLength) AND ($foundore = 0))
					setVar $result $result&"c r *q "
					setVar $waitPort 1
				end
				...
				setVar $result ""
				if ($waitPort = 1)
					setTextLineTrigger waitno :waitno "I have no information about a port in that sector."
					setTextLineTrigger waityes :waityes "-----     ------  ------- -------- -------"
					setTextLineTrigger waitclass0 :waitclass0 "A  Cargo holds"
					pause
					:waityes
						killalltriggers
						setTextLineTrigger ftrading :ftrading "Fuel Ore"
						pause
						:ftrading
							getWord CURRENTLINE $trade 3
	echo "TRADE: " $trade "*"
							if ($trade = "Selling")
								send "p t * * 0 * 0 * * "
								setVar $foundore 1
