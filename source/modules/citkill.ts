	...
	setVar $target ""
	if ($parm1 <> "on")
        	send "'{" $bot_name "} - Please use - citkill [on/off]*"
		halt
	else
		if ($startingLocation <> "Citadel")
			send "'{" $bot_name "} - Citadel Killer must be run from the Citadel Prompt*"
			setVar $mode "General"
			halt
		end
		isNumber $test $parm2
		...
		getWordPos $user_command_line $pos "empty"
		if ($pos > 0)
		...
		getWordPos $user_command_line $pos "smart"
		if ($pos > 0)
			setVar $smart TRUE
		else
			setVar $smart FALSE
...
:start
	send "'{" $bot_name "} - Citadel Killer :: Powering Up!*"
...
:warning
	...
	gosub :getPlanetInfo
	if ($targetingPerson)
		send "'{" $bot_name "} - Citadel Killer Targeting "&$target&" :: Running on Planet " $PLANET " :: " $PLANET_FIGHTERS " Fighters available on surface.*"
	elseif ($targetingCorp)
		send "'{" $bot_name "} - Citadel Killer Targeting Corp "&$target&" :: Running on Planet " $PLANET " :: " $PLANET_FIGHTERS " Fighters available on surface.*"
	else
		send "'{" $bot_name "} - Citadel Killer :: Running on Planet " $PLANET " :: " $PLANET_FIGHTERS " Fighters available on surface.*"
	end
	if ($shotgun)
...
:main
	killalltriggers
	gosub :quikstats
	setTextLineTrigger 	limp 	:scanit_cit_kill 	"Limpet mine in "&$CURRENT_SECTOR
	setTextLineTrigger 	warps 	:scanit_cit_kill 	"warps into the sector."
	setTextLineTrigger 	lifts 	:scanit_cit_kill 	"lifts off from"
	setTextLineTrigger 	deffig 	:scanit_cit_kill 	"Deployed Fighters Report Sector "&$CURRENT_SECTOR
...
:scanit_again
	...
	gosub :quikstats
	gosub :getSectorData
	if ($realTraderCount > ($corpieCount + $defenderShips))
	...
	if ($posShips > 0)
		...
		while ($temp <> "")
			...
			if ($pos2 > 0)
				...
				if (($EMPTYSHIPS[($emptyShipCount+1)] = $CORP) OR ($EMPTYSHIPS[($emptyShipCount+1)] = $TRADER_NAME))
					add $myShipCount 1
				end
...
:fastCitadelAttack
	...
	if ($FIGHTERS > 0)
		...
		gosub :quikstats
		if ($FIGHTERS <= 0)
			send "'{" $bot_name "} - Out of fighters, shutting down "&$command&".*"
			halt
		end
	end
	...
	if ($isFound = TRUE)
		setVar $thisKillTarget ""
		setVar $lastKillTarget ""
		if ($smart)
			setVar $attackString ""
			send "q "
			setVar $count 8
			while ($count > 0)
				if ($shotgun)
					...
					if ($doubletap)
						send $attackString $attackString&"q "&$targetString&$SHIP_MAX_ATTACK&"* * "&$targetString&$SHIP_MAX_ATTACK&"* * "&$refurbString
					else
						send $attackString $attackString&"q "&$targetString&$SHIP_MAX_ATTACK&"* * "&$refurbString
					end
				end
				...
				:foundkilltarget
					...
					setVar $thisKillTarget CURRENTANSILINE
					getWordPos $thisKillTarget $pos "[0;33m([1;36m"
					cutText $thisKillTarget $thisKillTarget 1 $pos
					getWordPos $thisKillTarget $pos "'s "
					while ($pos > 0)
						cutText $thisKillTarget $thisKillTarget ($pos+3) 9999
						getWordPos $thisKillTarget $pos "'s "
					end
					getText $thisKillTarget $thisKillTarget #27&"[0m"&#27 #27&"["
					getText $thisKillTarget&"/\ENDOFSHIPTAG/\" $thisKillTarget "m" "/\ENDOFSHIPTAG/\"
					getWordPos $TRADERS[($c-1)][1] $pos $thisKillTarget
					if ((($lastKillTarget <> "") AND ($thisKillTarget <> $lastKillTarget)))
						echo "*Target has changed, time to rescan..*"
						send " c "
						goto :doneKill
					end
					setVar $lastKillTarget $thisKillTarget
				:nokilltargets
					killalltriggers
...
:fastCaptureOld
	...
	#	end
	#end	
	if (($emptyshipcount > 0) AND ($CURRENT_SECTOR > 10) AND ($CURRENT_SECTOR <> STARDOCK))
		if ($startingLocation = "Citadel")
			setVar $targetString  "q a "
		else
			setVar $targetString  "a "
	...
	if ($isFound = FALSE)
		...
		:cap_ship
			...
			setVar $attack_count 0
		while (($PLANET_FIGHTERS > 0) AND ($attack_count < 2))
			add $attack_count 1
			...
			:ship_type
				...
				clientMessage "Unknown ship type, cannot calculate attack, you must do it manually.  Empty ship capturing shutting down feature shutting down.."
				setVar $capEmptyShips FALSE
				send "* "
				goto :nocappingtargets
				return
