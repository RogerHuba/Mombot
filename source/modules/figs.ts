	#
	# JAN 16 2017 - Fixed the Loadvar usage. And made the code for 'grid' report at end look prettier (eaier to read). 
	#
	logging off
...
:figs
	...
	elseif ($startingLocation = "Planet")
		...
		send "'{" $BOT~bot_name "} - Unknown Prompt*"
		halt
	end
...
:start_figs
	gosub :turnOffAnsi
	send "'{" $BOT~bot_name "} Loading current fighter locations. . .*"
	...
	SetVar $TEMP ("'*{" & $BOT~bot_name & "}*          - Fighter Grid Report -*          - ")
	SetVar $TEMP ($TEMP & $count & " sectors, " & $personalCount & " personal. (" & $percent & "%) (")
	SetVar $TEMP ($TEMP & $gridChange & " Change)*          - T: " & $tollCount & "  O: " & $offCount)
	SetVar $TEMP ($TEMP & "  D:" & $defCount & "*          - DE: " & $1sCountformatted & "" & $1percentformatted)
	SetVar $TEMP ($TEMP & " 2S: " & $2sCountformatted & $2percentformatted)
	SetVar $TEMP ($TEMP & " 3S: " & $3sCountformatted & $3percentformatted)
	SetVar $TEMP ($TEMP & "*          - 4S: " & $4sCountformatted & $4percentformatted)
	SetVar $TEMP ($TEMP & " 5S: " & $5sCountformatted & $5percentformatted)
	SetVar $TEMP ($TEMP & " 6S: " & $6sCountformatted & $6percentformatted)
	SetVar $TEMP ($TEMP & "*          - Upgraded Sxx: " & $upgradedFuelCount)
	SetVar $TEMP ($TEMP & " (" & $gridFuelChange & " Change)*          - Upgraded xBx: ")
	SetVar $TEMP ($TEMP & $upgradedOrgCount & " (" & $gridOrgChange & " Change)*          - Upgraded xxB: ")
	SetVar $TEMP ($TEMP & $upgradedEquipCount & " (" & $gridEquipChange & " Change)*          - Upgraded xxS: ")
	SetVar $TEMP ($TEMP & $upgradedEquipSellCount & " (" & $gridEquipSellChange & " Change)**")
	Send $TEMP
	...
	:corpCount
		...
		while ($i <= $sector)
			...
			if (($sector = $i) OR ($pos > 0))
				...
				setVar $tempWarpCount SECTOR.WARPCOUNT[$i]
				if ($tempWarpCount > 0)
					if ($tempWarpCount = 1)
						add $1sCount 1
