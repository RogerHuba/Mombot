	...
	setVar $BOT~help[18] $BOT~tab&"         [one] - only pops one planet where none exist during build"
...
:tryAgain
	while ($farmSector <= SECTORS)
		...
		:warp_it
			...
			if (($strip = TRUE) AND ($PLANET~PLANET_ORGANICS > ($PLANET~PLANET_ORGANICS_MAX-1000)) AND ($PLANET~PLANET_EQUIPMENT > ($PLANET~PLANET_EQUIPMENT_MAX - 1000)))
...
:remove_colos
	...
	while ($COLOS > $COLOS_MAX)
		...
		if ($cyclebuffer >= $cyclebufferlimit)
			...
			gosub :PLAYER~quikstats
			send "qq* l " & #8 & $planets[$j] & "*  s * t "&$type&$holds_to_grab&"*  q l " & #8 & $planetToFill & "*  snl1*"
...
:add_colos
	...
	while ($COLOS < $COLOS_MIN)
		...
		if ($cyclebuffer >= $cyclebufferlimit)
			...
			send "q q* l " & #8 & $planetToFill & "*  snt"&$type&"* q q* l " & #8 & $planets[$j] & "*  snl"&$add_type&"*"
		end
