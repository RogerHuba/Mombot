	...
	gosub :BOT~loadVars
		...
		setVar $BOT~help[1]  $BOT~tab&" ugrid [targeting] {figs} {armids} {limpets} {safety} {planets} {warp} " 
		setVar $BOT~help[2]  $BOT~tab&"       {refurb} {scrub} {avoid) {aggressive} {clear} {noansi} {ship#:#}" 
...
:load_script
	...
	setVar $i 2
	setVar $xport_ship_count 1
	setArray $xport_ships 100
	setVar $xport_ships[1] $ship_number
	getWordPos $user_command_line $pos "ship"&$i&":" 
	while ($pos > 0)
		getText " "&$user_command_line&" " $xport_ships[$i] "ship"&$i&":" " "
		setVar $xport_grid TRUE
		add $i 1
		add $xport_ship_count 1
		getWordPos $user_command_line $pos "ship"&$i&":" 
	end
...
:hittingsec
	...
	if (((($avoidShieldedOnly = TRUE) AND ($containsShieldedPlanet = FALSE)) OR (SECTOR.PLANETCOUNT[$boomsec] <= 0)) and (SECTOR.TRADERCOUNT[$boomsec] <= 0) and ($distance = 1) and ($boomsec > 10) and ($boomsec <> STARDOCK) and ((($attackretreat = TRUE) AND ($distanceback = 1) AND (SECTOR.FIGS.QUANTITY[$boomsec] >= ($offodd*2))) OR (SECTOR.FIGS.QUANTITY[$boomsec] < ($offodd*2))))
		...
		if ($xport_grid)
			setVar $entire_macro $boomsec&$attack_mac&$mac&$xport_mac&"**  "
			send $entire_macro
			gosub :quikstats
			if ($ship_number <> $xport_ship)
				send "'{" $bot_name "} - Didn't make it to xport ship, calling saveme.*"
				goto :callSaveMe
			end
			if ((SECTOR.PLANETCOUNT[$CURRENT_SECTOR] > 0) AND ($CURRENT_SECTOR <> $HOMESEC))
				send "'{" $bot_name "} - Planet in xport ship sector, calling saveme.*"
				goto :callSaveMe
			end
			if ((SECTOR.TRADERCOUNT[$CURRENT_SECTOR] > 0) AND ($CURRENT_SECTOR <> $HOMESEC))
				send "'{" $bot_name "} - Trader in xport ship sector, calling saveme.  Please kill them.*"
				goto :callSaveMe
			end
			send $return_mac
		else
...
:assemble_xport_mac
	setVar $xport_mac ""
	if ($xport_grid)
		gosub :get_next_xport_ship
		setVar $xport_mac "x   "&$xport_ship&"*  *  "
	end
return
...
:get_next_xport_ship
	setVar $i 1
	setVar $xport_ship $ship_number
	while ($i <= $xport_ship_count)
		if ($xport_ships[$i] = $ship_number)
			if ($i >= $xport_ship_count)
				setVar $xport_ship $xport_ships[1]
			else
				setVar $xport_ship $xport_ships[($i+1)]
			end
			return
		end
		add $i 1
