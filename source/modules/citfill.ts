...
:FigMe
	...
	if ($parm1 <> "on")
        	send "'{" $bot_name "} - Please use - citfill [on/off]*"
		halt
	else
		if ($startingLocation <> "Citadel")
			send "'{" $bot_name "} - This mode must be run from the Citadel Prompt*"
			setVar $mode "General"
			halt
		end
		isNumber $test $parm2
		...
		if ($parm2 <= 0)
			...
			setVar $figsToRefill $parm2
		end

		:start_cit_fill
			send "'{" $bot_name "} - Citadel Ship Re-Filler :: Powering Up!*"
		...
		:warning_cit_fill
			...
			send "'{" $bot_name "} - Citadel Ship Re-Filler :: Running on Planet " $planetinfo~PLANET " :: " $planetinfo~PLANET_FIGHTERS " Fighters available on surface.*"
			send "'{" $bot_name "} - Citadel Ship Re-Filler now active! Script will re-fig an ally in the sector over planet " & $planetinfo~PLANET & ".*"
			send "'{" $bot_name "} - Citadel Ship Re-Filler will attempt to refill "&$figsToRefill&" fighters at a time.*"
	end
	goto :settriggers
...
# Kaboom deploys some fighters.
	...
	send "q q t "&$reloadNow&$reloadNow&$reloadNow&" * l " & $planetinfo~PLANET & "* m * * *  c "
	goto :settriggers
