	...
	if ($doesHelpFileExist <> TRUE)
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "- stripall {set} {clear} {list} {f} {o} {e} {fig}                                " 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "    Visits sectors in list and strips the planets there.     " 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "       - default will visit all planets on the tl list       " 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "                                                            " 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "    [set] {sector1} {sector2} {...} {sectorx}               "
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "       - set puts sectors in the order you enter into a file" 
		...
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "       " 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "    {personal}   " 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "       - goes to all personal planets instead of corp" 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "       " 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "   {f} - strips fuel ore" 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "   {o} - strips organics" 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "   {e} - strips equipment" 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "   {fig} - strips fighters" 
		write "scripts\"&$bot~mombot_directory&"\help\"&$command&".txt" "       - default is all" 
	...
	if ($pos > 0)
		...
		send "'{" $bot_name "} - Bot Stripping File has been deleted.*"
		halt
	end

	getWordPos " "&$user_command_line&" " $pos " personal " 
	if ($pos > 0)
	...
	if ($pos > 0)
		...
		if ($test)
			...
			setVar $list_output $list_output&$sector[$i]
			send "'*{" $bot_name "} - Planet Stripping List (In traveling order) *"&$list_output&"**"
		else
			send "'{" $bot_name "} - No Planet Stripping File to list from.*"
		end
		halt
	end
	...
	if ($pos > 0)
		...
		send "'{" $bot_name "} - "&($i-2)&" Sectors added to Bot Planet Stripping File.*"
		halt
	end
	setVar $i 1
	...
	fileExists $test $STRIPPER_FILE
	if ($test)
		send "'{" $bot_name "} - Loading Planet List From Planet Stripping File...*"
		readToArray $STRIPPER_FILE $sector 
	else
		setVar $sector SECTORS
		setArray $sector SECTORS
		send "'{" $bot_name "} - No Planet Stripping File, Loading Planet List...*"
		gosub :get_tl_list
	end

	send "'{" $bot_name "} - Planet List Loaded, starting the Planet Stripping!*"	
	gosub :quikstats
...
:tryAgain
	while ($i <= $sector)
		...
		:warp_it
			...
			if ($silent <> TRUE)
				send "'{" $bot_name "} - Done Planet Stripping sector " $sector[$i] ".*"
			end
			send "q"
			gosub :getPlanetInfo
			send "c"
...
send "p "&$home&"  *ys* "
if ($planetIsFull)
	send "'{" $bot_name "} - Planet Stripping Planet is full.  Ready to sell off the product!*"
else
	send "'{" $bot_name "} - Planet Stripping run is complete.*"
end
gosub :quikstats
