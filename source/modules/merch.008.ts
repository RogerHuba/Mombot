...
:merchant
	...
	if (($sellingOrg = FALSE) AND ($sellingEquip = FALSE))
		send "'{" $bot_name "} - Please pick [o]rganics and/or [e]quipment to sell.  merch [min product] {o} {e} {skipcim} {negotiate/hold}*"
		halt
	end
...
:merchant
	...
	send "c"
	if ($citadel < 4)
		send "'{" $bot_name "} - You must run Planet Merchant from at least a level 4 planet.*"
     		halt
	end
	gosub :quikstats
	...
	while ((($sellingOrg) AND ($planetorg >= 500)) OR (($sellingEquip) AND ($planetequip >= 500)))
		...
		while ($bottom <= $top)
			...
			if (($docim = FALSE) AND ($skipcim = FALSE))
				if (($checkedPorts[$focus] <> TRUE) AND (PORT.EXISTS[$focus] = TRUE) AND (PORT.CLASS[$focus] > 0) AND (SECTOR.EXPLORED[$focus] = "YES") AND ((($sellingOrg = TRUE) AND ($planetorg > 500) AND (PORT.BUYORG[$focus])) OR (($sellingEquip = TRUE) AND ($planetequip > 500) AND (PORT.BUYEQUIP[$focus]))))
					send "cr"&$focus&"*q"
					gosub :quikstats
				end
			end
		...
		:doneMerchant
			...
			send "'{" $bot_name "} - Planet Merchant completed.*"
			halt
...
return
	...
	:Discod
	   	setVar $TagLine				"[Planet Merchant]"
		setVar $TagLineB			"[Planet Merchant]"
		killAllTriggers
	   	:Disco_Test
		if (CONNECTED <> TRUE)
