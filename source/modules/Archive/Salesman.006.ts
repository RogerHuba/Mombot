...
:merchant
	...
	while (TRUE)
		...
		while ($bottom <= $top)
			...
			if (($checkedPorts[$focus] <> TRUE) AND (PORT.EXISTS[$focus]) AND (PORT.CLASS[$focus] > 0) AND ((PORT.FUEL[$focus] >= $minimumFuel) OR (PORT.ORG[$focus] >= $minimumFuel) OR (PORT.EQUIP[$focus] >= $minimumFuel)) AND ($isBusted <> TRUE))
				# fig found 0 hops
