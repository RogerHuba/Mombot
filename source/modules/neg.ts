	#$MCIC_FILE value was missing, script otput data to a file call '0' (zero).
	logging off
	loadVar $bot_name
...
:neg
	...
	gosub :planetNeg
	setTextTrigger done_buy :done_neg "Planet Negotiation exiting"
...
:initinfo
    ...
    setVar $fueltosell $PLANET_FUEL
    if ($fueltosell > $PLANET_FUEL)
        setVar $fueltosell $PLANET_FUEL
    end
    ...
    setVar $orgtosell $PLANET_ORGANICS
    if ($orgtosell > $PLANET_ORGANICS)
        setVar $orgtosell $PLANET_ORGANICS
    end
    ...
    setVar $equiptosell $PLANET_EQUIPMENT
    if ($equiptosell > $PLANET_EQUIPMENT)
        setVar $equiptosell $PLANET_EQUIPMENT
    end
...
:selloff
    if (($fueltosell <> 0) or ($orgtosell <> 0) or ($equiptosell <> 0))
        ...
        write $MCIC_FILE $generalOutput

        if ($oreselloutput <> "")
            send $oreselloutput
            write $MCIC_FILE $oreselloutput
        end
        if ($orgselloutput <> "")
            send $orgselloutput
            write $MCIC_FILE $orgselloutput
        end
        if ($equselloutput <> "")
            send $equselloutput
            write $MCIC_FILE $equselloutput
        end
