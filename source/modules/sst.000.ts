:load_variables
	...
	loadVar $unlimitedGame
        IF ($parm1 = "help")
               send "'*{" $bot_name "} - sst [ship1] [ship2] {jet}*"
               send "   - Do NOT need to start in Ship 1 or Ship 2.*"
               send "   - First Steal will be from Ship 1.*"
               send "   - {jet} will mega jet product for extra experience*"
               send "   -*   -Created by Cherokee**"
               HALT
        END
        isNumber $test $parm1
        IF ($test)
        ...
        setVar $ship_1 $parm1
        setVar $ship_2 $parm2
        setVar $steal_divisor $steal_factor
        IF ($parm3 = "jet")
                setVar $jet "y"
        END
        ...
        if ($jet = "y")
            if ($port[$current_ship] = 3) or ($port[$current_ship] = 4)
                ...
                elseif ($port[$current_ship].ore_selling > 0)
                    ...
                    send "'{" $bot_name "} - This port is selling 0 ore, wait for it to regen to at least 1 unit, then restart.*"
                    halt
                end
            end
            if ($port[$current_ship] = 2) or ($port[$current_ship] = 4)
                ...
                elseif ($port[$current_ship].org_selling > 0)
                    ...
                    send "'{" $bot_name "} - This port is selling 0 org, wait for it to regen to at least 1 unit, then restart.*"
                    halt
                end
            end
            send "JY"
        ...
        # no jet
            ...
            if ($port[$current_ship] = 2) or ($port[$current_ship] = 4)
                if ($port[$current_ship].org_selling > 0)
                    ...
                    send "'{" $bot_name "} - his port is selling 0 org, wait for it to regen to at least 1 unit, then restart.*"
                    halt
                end
            end
            send "JY"
