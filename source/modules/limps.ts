...
:start_limps
	gosub :turnOffAnsi
	send "'{" $bot_name "} Loading current limpet locations. . .*"
	fileExists $gfile_chk $LIMP_COUNT_FILE
	if ($gfile_chk = 1)
		read $LIMP_COUNT_FILE $previousCount 1
	else
	...
	send "'*{"&$bot_name&"}*          - Limpet Grid Report -*          - "&$count&" sectors, "&$personalCount&" personal. ("&$percent&"%) ("&$gridChange&" Change)*          - Activated  Limpet  Scan*            *             Sector    Personal/Corp*            ========================*"&$limpetOutput&"*"
...
:refreshLimps
	...
	:checkMarkedLimps
		...
		delete $LIMP_FILE
		write $LIMP_FILE $output
		delete $LIMP_COUNT_FILE
		write $LIMP_COUNT_FILE $count
return
