...
:fastCapture
    ...
    if (($isFound = FALSE) AND ($SECTOR~emptyShipCount > 0) AND ($CURRENT_SECTOR > 10) AND ($CURRENT_SECTOR <> $MAP~STARDOCK))
        ...
        setVar $c 1
        setVar $isFound FALSE
        while (($c <= $SECTOR~emptyShipCount) AND ($isFound = FALSE))
...
:surround
    :StartSurround
        ...
        if ($SHIP~SHIP_MAX_ATTACK > $FIGHTERS)
            setVar $SHIP~SHIP_MAX_ATTACK ($FIGHTERS/2)
        end
        ...
        setvar $totalWarps SECTOR.WARPCOUNT[$current_sector]
        while (SECTOR.WARPS[$CURRENT_SECTOR][$i] > 0)
            ...
            elseif (($containsShieldedPlanet = TRUE) AND ($surroundAvoidShieldedOnly = TRUE))
...
:moveIntoSector
    ...
    if ($SHIP~SHIP_MAX_ATTACK <= 0)
        ...
        setVar $attack $SHIP~SHIP_MAX_ATTACK&"9999"
    end
...
:holo_kill_get_current_sector
        setVar $hkill_start_sector $CURRENT_SECTOR
        setVar $killsector 0
        setVar $idx 1
...
:holo_kill_killem
        ...
        gosub :quikstats
        if ($CURRENT_SECTOR <> $hkill_start_sector)
