	...
	setVar $BOT~help[1]  $BOT~tab&"mslingshot [planettarget] [invasion_bots] {waves} {wavesize}*"
	setVar $BOT~help[2]  $BOT~tab&"[planettarget]  - Planet we will attempt to land on"
	...
	setVar $attackplan "We are sending callin:" & $invasionBots & " at planet:"  & $targetPlanet
	if ($waves > 0)
		setVar $attackplan $attackplan & " firing " & $waves & " wave(s) of " & $figsperwave & " at the planet."

	end
	setVar $attackplan $attackplan & "*"
	setVar $SWITCHBOARD~message $attackplan
	gosub :SWITCHBOARD~switchboard
	

	setVar $invasion_mac " mac Q Q m SECTOR ^m n n z ^M a  z 9999 ^m a z 9999 ^m ^m ^m ^m ^m z ^m l"&$targetPlanet&"^mn n ^m ^m " & $wavesMacro &" *"

	setVar $invasions_bots_macro "'" & $invasionBots &  $invasion_mac
	setVar $invasions_bots_macro_test "'InvasionMacro: " & $invasion_mac

	setVar $SWITCHBOARD~message $invasions_bots_macro_test & "*"
	gosub :SWITCHBOARD~switchboard
...
echo $w ":" SECTOR.WARPSIN[$dropSector][$w] " " $isFigged " *"
			...
			send $front_attack_mac&$adjSector&$mid_attack_mac&$dropSector&$enter_attack_mac&"'"&$dropSector&"=saveme*"&$deploy_fig_mac 
			setVar $i 0
			while ($i < 10)
				add $i 1
				send "l  j" & #8 & $planet & "*  *  "
